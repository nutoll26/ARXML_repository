...
public class QueryForRecordSheet {
	static String queryEnumerationSheet(String ecuName){
		String query = 
				...
				+ "    let $recordDataShortName := $recordDataType[$i]/SHORT-NAME\n"
				+ "    let $recordDataLongName := $recordDataType[$i]/LONG-NAME\n"
				+ "    let $recordDataDescription := $recordDataType[$i]/DESC\n"
				+ "    let $numOfType := count($recordDataType[$i]/ELEMENTS/APPLICATION-RECORD-ELEMENT)\n"
				+ "    let $valueList :=\n"
				+ "    <tag>{\n"
